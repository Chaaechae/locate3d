...
# misc custom setting
# DINOv3 ViT uses patch_size=16, so the crop must be divisible by 16.
# 512 / 16 = 32 -> 32x32 = 1024 patch tokens (patch_h == patch_w == 32).
# Keeping crop == patch_h * patch_size guarantees ImgAugmentation crop_start == [0, 0],
# i.e. the 3D->2D correspondence grid stays exactly aligned end-to-end.
crop_h = 512
crop_w = 512
patch_size = 16
...
model = dict(
    ...
    # DINOv3 ViT-H+/16 (registers are built-in; no separate "-reg" variant exists).
    # The name only needs to avoid the "radio" branch in ENC2D_forward; the loader
    # uses image_weight_path. last_hidden_state layout is [CLS, reg x4, patch...],
    # and ENC2D_forward slices the trailing patch tokens, so it works unchanged.
    image_weight_name="dinov3_vith16plus",
    image_weight_path="/group-volume/dinov3-vith16plus-pretrain-lvd1689m",
    ...
    enc2d_head_in_channels=1280,  # DINOv3 ViT-H+/16 hidden_size (was 1536 for DINOv2-giant)
    ...
    train=dict(
        ...
        datasets=[
            ...
            dict(
                ...
                data_root="/group-volume/3Ddataset/data/waymo",
            ...
            dict(
                ...
                patch_size=patch_size,
                split=["train"],
                data_root="/group-volume/3Ddataset/data/partnet_data_v0",
            ...
            dict(
                ...
                data_root="/group-volume/3Ddataset/data/scannet",
                transform=indoor_transform,
                test_mode=False,
                loop=1,
            ),
            # Structured3D: excluded — its coord.npy and correspondence point indices
            # are mismatched (different point clouds), so it is disabled until the data
            # is regenerated with a matched preprocessing.
        ],
    )
)
